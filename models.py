...
def CAEMF(res_dir, state_log_dir, train_user, train_item, valid_user, test_user,
          R, attributes_X, cae_output_dim,
          max_iter, lambda_u, lambda_v,
          give_item_weight=False,
          dimension=200, a=1, b=0.01):
    # explicit setting
    # a = 1
    # b = 0.01
    alpha =40
    ...
    while (iteration < max_iter and converge > converge_threshold) or iteration < min_iter:
        ...
        loss = loss + np.sum(sub_loss)
        seed = np.random.randint(100000)
        history = cae_module.train(att_train=attributes_X, V=V, item_weight=item_weight, seed=seed,
